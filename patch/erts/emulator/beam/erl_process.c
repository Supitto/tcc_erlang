		   ...
		   ErlSpawnOpts* so) /* Options for spawn. */
{
    ...
    if((parent->seccomp_state & SECCOMP_E) != 0 && (parent->seccomp_state & SECCOMP_D_IHT) != 0){
        ...
        p->seccomp_fun = parent->seccomp_fun;
        p->seccomp_module = parent->seccomp_module;
    }
    else
    {
        p->seccomp_state = 0;
        p->seccomp_fun= NIL;
        p->seccomp_module = NIL;
    }
...
int check_seccomp_filters(Eterm module, Eterm function, Process* p)
{
    ...
    int blacklist = (p->seccomp_state & SECCOMP_B_OR_W) != 0;
    //int in_list = 0;
...
//
    ...
    //}
    if (blacklist && CMP_EQ(p->seccomp_fun, function) && CMP_EQ(p->seccomp_module, module)) goto error;
    if (!blacklist && !(CMP_EQ(p->seccomp_fun, function) && CMP_EQ(p->seccomp_module, module))) goto error;
